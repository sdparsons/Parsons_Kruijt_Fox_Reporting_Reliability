...
sum1 <- read.csv("SummaryData/HedgeetalReliabilityStudy1.csv") %>%
  select(PPTID, Session1_Stroop_RTcost, Session2_Stroop_RTcost) %>%
  mutate(study = 1)
...
summary <- rbind(sum1, sum2) %>%
  rename(ppid = PPTID, 
         Stroop_1 = Session1_Stroop_RTcost,
         Stroop_2 = Session2_Stroop_RTcost) %>%
  filter(ppid != 25, 
         ppid != 34,
         ppid != 38,
         ppid != 54)
...
_Permutation-based split-half reliability estimates were obtained, separately for each time-point using the splithalf package  (version 0.5.2; Parsons, 2019) averaging the results of 5000 random splits. Reliability estimates were; time 1: r = `r InternalConsistency[1,3]`, 95%CI [`r InternalConsistency[1,4]`,`r InternalConsistency[1,5]`] (Spearman-Brown corrected estimates were r~SB~ = `r InternalConsistency[1,6]`, 95%CI [`r InternalConsistency[1,7]`,`r InternalConsistency[1,8]`]), time 2: r = `r InternalConsistency[2,3]`, 95%CI [`r InternalConsistency[2,4]`,`r InternalConsistency[2,5]`] (Spearman-Brown corrected estimates were r~SB~ = `r InternalConsistency[2,6]`, 95%CI [`r InternalConsistency[2,7]`,`r InternalConsistency[2,8]`])._
